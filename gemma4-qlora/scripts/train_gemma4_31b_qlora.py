...
def should_enable_unsloth_patch(argv: list[str]) -> bool:
    loader_type = "fast_vision"
    for index, value in enumerate(argv):
        if value == "--model-loader-type" and index + 1 < len(argv):
            loader_type = argv[index + 1]
            break
        if value.startswith("--model-loader-type="):
            loader_type = value.split("=", 1)[1]
            break
    return loader_type.startswith("fast_")


if should_enable_unsloth_patch(sys.argv):
...
from runtime.bnb_quant_state import restore_bnb_4bit_quant_states  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--prepare-kbit-training", action=argparse.BooleanOptionalAction, default=True)
...
def resolve_text_processing_class(processor: Any) -> Any:
...
def apply_peft_model(loader: Any, model: Any, args: argparse.Namespace) -> Any:
    if args.model_loader_type.startswith("hf_"):
        ...
        for name, module in model.named_modules():
            if name.rsplit(".", 1)[-1] not in candidate_modules:
                continue
            weight = getattr(module, "weight", None)
            if hasattr(weight, "compress_statistics") and hasattr(weight, "quant_type"):
                target_modules.append(name)
        if not target_modules:
            raise ValueError("No compatible 4bit Linear modules found for HF LoRA injection")
        print("HF_LORA_TARGETS", len(target_modules), sorted({item.rsplit('.', 1)[-1] for item in target_modules}), flush=True)
        config = LoraConfig(
            r=args.lora_r,
...
def main() -> None:
    ...
    if args.model_loader_type.startswith("hf_") and args.model_load_in_4bit:
        restored_after_peft = restore_bnb_4bit_quant_states(model, args.model_dir)
        print(f"BNB_4BIT_QUANT_STATE_RESTORED_AFTER_PEFT {restored_after_peft}", flush=True)
    ...
    elif args.model_loader_type == "hf_vision" and args.finetune_scope != "language_only":
        raise ValueError("hf_vision image/video fine-tuning needs a dedicated VLM collator; use language_only until VLM datasets are enabled.")
    else:
        trainer_dataset = prepare_language_dataset(dataset, processor, chat_template_kwargs)
        processing_class = resolve_text_processing_class(processor)
