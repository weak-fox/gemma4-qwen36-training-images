...
from runtime.bnb_quant_state import restore_bnb_4bit_quant_states
...
def build_hf_from_pretrained_kwargs(
    *,
    ...
    if model_load_in_4bit:
        kwargs["quantization_config"] = build_bnb_4bit_config(
            model_4bit_quant_type,
            model_4bit_compute_dtype,
        )
    return kwargs
...
def get_hf_model_class(model_loader_type: str) -> Any:
    if model_loader_type == "hf_language":
        from transformers import AutoModelForCausalLM

        return AutoModelForCausalLM
    if model_loader_type == "hf_vision":
        try:
...
def load_unsloth_model(
    *,
    ...
    if model_loader_type.startswith("hf_"):
        from transformers import AutoProcessor, AutoTokenizer

        model_class = get_hf_model_class(model_loader_type)
        ...
        model = call_with_supported_kwargs(model_class.from_pretrained, **kwargs)
        restored_quant_states = restore_bnb_4bit_quant_states(model, model_name_or_path)
        print(f"BNB_4BIT_QUANT_STATE_RESTORED {restored_quant_states}", flush=True)
        if model_loader_type == "hf_language":
            processor = AutoTokenizer.from_pretrained(model_name_or_path, local_files_only=local_files_only)
        else:
            try:
                processor = AutoProcessor.from_pretrained(model_name_or_path, local_files_only=local_files_only)
            except Exception:
                processor = AutoTokenizer.from_pretrained(model_name_or_path, local_files_only=local_files_only)
        return None, model, processor
