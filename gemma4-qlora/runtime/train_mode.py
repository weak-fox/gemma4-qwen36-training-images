...
def _build_train_command(settings: TrainingSettings, dataset_path: Path) -> list[str]:
    command = [
        ...
        "--model-loader-type",
        settings.model_loader_type,
        "--model-4bit-quant-type",
        settings.model_4bit_quant_type,
        "--model-4bit-compute-dtype",
        settings.model_4bit_compute_dtype,
        "--finetune-scope",
        settings.finetune_scope,
        "--precision",
        settings.train_precision,
        "--output-dir",
    ...
    if settings.model_chat_template_kwargs:
        import json

        command.extend(
            [
                "--model-chat-template-kwargs-json",
                json.dumps(settings.model_chat_template_kwargs, ensure_ascii=False),
            ]
        )
    ...
    if settings.model_load_in_4bit:
        command.append("--model-load-in-4bit")
    else:
        command.append("--no-model-load-in-4bit")
    if settings.save_adapter:
        command.append("--save-adapter")
    if settings.prepare_kbit_training:
        command.append("--prepare-kbit-training")
    else:
        command.append("--no-prepare-kbit-training")
