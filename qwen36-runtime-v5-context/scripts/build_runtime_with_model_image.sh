...
IMAGE_NAME="${IMAGE_NAME:-gemma4-qlora}"
IMAGE_TAG="${IMAGE_TAG:-runtime-with-model}"
if [[ "${IMAGE_TAG}" == *:* ]]; then
...
MODEL_DIR_NAME="${MODEL_DIR_NAME:-gemma-4-31b-it-unsloth-bnb-4bit}"
MODEL_DEST_DIR="/opt/models/${MODEL_DIR_NAME}"
CONTAINER_NAME="${IMAGE_NAME//[^a-zA-Z0-9_.-]/-}-runtime-with-model-$$"
...
echo "Created ${TARGET_IMAGE} from ${SOURCE_IMAGE}"
